...
The main objective of the ROCratePCC is to manage communication between in-memory ARC representation and on-disc representation. This is done using `Contract`s.
...
/// Functions found here might be moved to a ROCratePCC.NETIO Implementation
module ARC_IO =
    open FsSpreadsheet
    ...
    /// Create and fullfill READ contracts for ROCratePCC in .NET, then init ARC from existing ARC on disc.
    let readARC (arcRootPath: string) =
        readFilePaths arcRootPath               // Get all file paths relative to ARC root
        |> ARC.getReadContracts                 // Get READ contracts from ROCratePCC
