    ...
    let runTestsJsNative = BuildTask.createFn "runTestsJSNative" [clean; transpileTS] (fun tp ->
        if tp.Context.Arguments |> List.exists (fun a -> a.ToLower() = skipTestsFlag.ToLower()) |> not then
            ...
            for path in ProjectInfo.jsTestProjects do
                ...
                // run dotnet $"fable src/ROCratePCC/ROCratePCC.Javascript.fsproj -o {path}/ts --lang ts -e fs.ts --nocache" ""
                // System.IO.File.Copy("src/ROCratePCC/index.ts", $"{path}/index.ts", overwrite = true) |> ignore
                run npx $"vitest run --dir {path}" ""
            Trace.traceImportant "Start JavaScript web bundling of ROCratePCC"
    ...
    let runTestsPyNative = BuildTask.createFn "runTestsPyNative" [clean; transpilePy] (fun tp ->
        if tp.Context.Arguments |> List.exists (fun a -> a.ToLower() = skipTestsFlag.ToLower()) |> not then
            ...
            for path in ProjectInfo.pyTestProjects do
                // transpile library for native access
                //run dotnet $"fable src/ROCratePCC/ROCratePCC.Python.fsproj -o {path}/ROCratePCC/py --lang python --nocache" ""
                //System.IO.File.Copy("src/ROCratePCC/ROCratePCC.py", $"{path}/ROCratePCC/ROCratePCC.py", overwrite = true)
